...
namespace :light do
  ...
  task :remove_bounced_emails => :environment do
    ...
    Light::User.where(:email_id.in => bounce_emails).update_all(sidekiq_status: 'Bounced')
    Light::User.where(:email_id.in => invalid_emails).update_all(sidekiq_status: 'Invalid')
    Light::User.where(:email_id.in => spam_emails).update_all(sidekiq_status: 'Spam')
    Light::User.where(:email_id.in => block_emails).update_all(sidekiq_status: 'Block')
    #clean sendgrid... 
